...
import { Amount, assertAmountMatchesToken, fromAddress, resolveWalletAddress, } from "../types/index.js";
...
const DEFAULT_FROM_POOL_TIMEOUT_MS = 20000;
/**
...
export class Staking {
    constructor(pool, token, provider) {
        this.pool = pool;
        this.token = token;
     ...
     */
    populateEnter(walletAddress, amount) {
        assertAmountMatchesToken(amount, this.token);
        const tokenContract = this.tokenContract(this.provider);
        const approveCall = tokenContract.populateTransaction.approve(this.pool.address, amount.toBase());
        const enterPoolCall = this.pool.populateTransaction.enter_delegation_pool(walletAddress, amount.toBase());
     ...
     * ```
     */
    async enter(wallet, amount, options) {
        assertAmountMatchesToken(amount, this.token);
        if (await this.isMember(wallet)) {
            throw new Error(`Wallet ${wallet.address} is already a member in pool ${this.pool.address}`);
        }
     ...
     * ```
     */
    async stake(wallet, amount, options) {
        assertAmountMatchesToken(amount, this.token);
     ...
     * ```
     */
    async getPosition(walletOrAddress) {
     ...
     * @internal Used by {@link TxBuilder} — not part of the public API.
     */
    populateAdd(walletAddress, amount) {
        assertAmountMatchesToken(amount, this.token);
        const tokenContract = this.tokenContract(this.provider);
        const approveCall = tokenContract.populateTransaction.approve(this.pool.address, amount.toBase());
        const addPoolCall = this.pool.populateTransaction.add_to_delegation_pool(walletAddress, amount.toBase());
     ...
     * ```
     */
    async add(wallet, amount, options) {
        assertAmountMatchesToken(amount, this.token);
     ...
     * @internal Used by {@link TxBuilder} — not part of the public API.
     */
    populateClaimRewards(walletAddress) {
     ...
     * ```
     */
    async claimRewards(wallet, options) {
     ...
     * @internal Used by {@link TxBuilder} — not part of the public API.
     */
    populateExitIntent(amount) {
        assertAmountMatchesToken(amount, this.token);
        return this.pool.populateTransaction.exit_delegation_pool_intent(amount.toBase());
    }
    /**
     ...
     * ```
     */
    async exitIntent(wallet, amount, options) {
        assertAmountMatchesToken(amount, this.token);
        const member = await this.assertIsMember(wallet);
        ...
        if (member.staked.lt(amount)) {
            throw new Error(`Staked amount ${member.staked.toFormatted()} is lower than exiting intent amount.`);
        }
     ...
     * @internal Used by {@link TxBuilder} — not part of the public API.
     */
    populateExit(walletAddress) {
        return this.pool.populateTransaction.exit_delegation_pool_action(walletAddress);
    }
    /**
     ...
     */
    tokenContract(providerOrAccount) {
        return new Contract({
            abi: ERC20_ABI,
     ...
     * @throws Error if the wallet is not a member of the pool
     */
    async assertIsMember(wallet) {
        const maybeMember = await this.getPosition(wallet);
        if (!maybeMember) {
            throw new Error(`Wallet ${wallet.address} is not a member in pool ${this.pool.address}`);
        }
        return maybeMember;
    }
    /**
     ...
     * ```
     */
    static async fromPool(poolAddress, provider, config, options = {}) {
        ...
        const runStep = async (label, operation) => {
            ...
            return withTimeout(operation(), remainingMs, `Staking.fromPool timed out while ${label}`, options.signal);
        };
        ...
        const poolParameters = await runStep("loading pool parameters", () => poolContract.contract_parameters_v1());
        const stakerAddress = fromAddress(poolParameters.staker_address);
        const stakingContractAddressFromPool = fromAddress(poolParameters.staking_contract);
        ...
        const staker = await runStep("loading staker pool info", () => stakingContract.staker_pool_info(stakerAddress));
        ...
        const token = await runStep("resolving token metadata", () => getTokensFromAddresses([fromAddress(pool.token_address)], provider).then((tokens) => {
            return tokens[0];
        }));
        if (!token) {
            throw new Error(`Could not resolve token ${pool.token_address} in Pool ${poolAddress}`);
        }
     ...
     * ```
     */
    static async fromStaker(stakerAddress, token, provider, config) {
        const stakingContract = new Contract({
            abi: STAKING_ABI,
        ...
        if (!pool) {
            throw new Error(`No pool exists by staker ${stakerAddress} for ${token.symbol}`);
        }
        const poolContract = new Contract({
            abi: POOL_ABI,
     ...
     * ```
     */
    static async activeTokens(provider, config) {
        const stakingContract = new Contract({
            abi: STAKING_ABI,
            address: config.contract,
            providerOrAccount: provider,
     ...
     * ```
     */
    static async getStakerPools(provider, stakerAddress, config) {
        const stakingContract = new Contract({
            abi: STAKING_ABI,
            address: config.contract,
            providerOrAccount: provider,
