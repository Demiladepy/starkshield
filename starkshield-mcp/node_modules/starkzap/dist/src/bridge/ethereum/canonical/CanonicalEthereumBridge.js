...
import { ethereumAddress } from "../../../bridge/ethereum/EtherToken.js";
...
export class CanonicalEthereumBridge extends EthereumBridge {
    ...
    async getDepositFeeEstimate(_options) {
        ...
        const [allowance, l1ToL2MessageFee, approvalFeeEstimation] = await Promise.all([
            this.getAllowance(),
            this.estimateL1ToL2MessageFee(CanonicalEthereumBridge.DUMMY_SN_ADDRESS, minimalAmount),
        ...
        if (needsFallback) {
            const feeDecimal = CanonicalEthereumBridge.DEFAULT_ESTIMATED_DEPOSIT_GAS_REQUIREMENT *
                (await this.getEthereumGasPrice());
            l1Fee = this.ethAmount(feeDecimal);
        }
        else {
            const details = await this.prepareDepositTransactionDetails(CanonicalEthereumBridge.DUMMY_SN_ADDRESS, minimalAmount);
            const tx = await this.populateTransaction(details);
...
//# sourceMappingURL=CanonicalEthereumBridge.js.map