...
import { DCA_CONTINUOUS_FREQUENCY, } from "../dca/interface.js";
import { isRecord } from "../utils/ekubo.js";
...
function getEkuboTimeStep(now, time) {
...
export function assertFitsU128(value, label) {
    if (value < 0n || value > MAX_U128) {
        throw new Error(`${label} must fit in u128`);
    }
}
export function toEkuboApiChainId(chainId) {
    return BigInt(chainId.toFelt252()).toString(10);
}
export function parsePositiveBigInt(value, label) {
    let parsed;
    try {
        ...
        return {
            ...
            orders: group.orders.map((order) => {
                ...
                return {
                    ...
                    total_proceeds_withdrawn: parseRequiredString(order.total_proceeds_withdrawn, "total_proceeds_withdrawn"),
                    sale_rate: parseRequiredString(order.sale_rate, "sale_rate"),
                    last_collect_proceeds: order.last_collect_proceeds == null
                        ? null
                        : parseRequiredNumber(order.last_collect_proceeds, "last_collect_proceeds"),
                    total_amount_sold: parseRequiredString(order.total_amount_sold, "total_amount_sold"),
                };
            }),
        };
    });
...
export function parseIsoDurationSeconds(value) {
    ...
    if (!Number.isFinite(totalSeconds) || totalSeconds <= 0) {
        throw new Error(`DCA frequency must resolve to a positive duration: ${value}`);
    }
    return totalSeconds;
}
export function alignEkuboTime(now, target) {
    let candidate = Math.max(target, now + EKUBO_TIME_SPACING_SECONDS, now + MINIMUM_START_DELAY_SECONDS);
...
export function decodeEkuboOrderId(orderId) {
    const parts = orderId.split(":");
    if (parts.length !== 8 || parts[0] !== ORDER_ID_PREFIX) {
        throw new Error(`Invalid Ekubo DCA order id "${orderId}". Expected ${ORDER_ID_PREFIX}:<positions>:<tokenId>:<sellToken>:<buyToken>:<fee>:<startTime>:<endTime>.`);
    }
    return {
...
export function toOrderInfoCalldata(order) {
    return [
...
export function parseOrderInfoResult(result) {
    if (result.length !== 3) {
        throw new Error("Ekubo order info response is malformed");
    }
    return {
        saleRate: parsePositiveBigInt(result[0], "sale_rate"),
        remainingSellAmount: parsePositiveBigInt(result[1], "remaining_sell_amount"),
 ...
 */
export function parseOrderInfosResult(result, expected) {
    const values = result.map((item) => String(item));
    const expectedLength = expected * 3 + 1;
    if (values.length !== expectedLength) {
        throw new Error(`Ekubo order infos response is malformed: expected ${expectedLength} values, got ${values.length}`);
    }
    const declaredLength = Number(values[0]);
    if (declaredLength !== expected) {
        throw new Error(`Ekubo order infos length mismatch: header says ${declaredLength}, expected ${expected}`);
    }
    const infos = [];
...
export function pickTwammPoolFee(payload, twammExtension) {
    const matchingPool = payload.topPools.find((pool) => fromAddress(pool.extension) === twammExtension);
    if (!matchingPool) {
...
export function toEkuboDcaOrder(params) {
    ...
    const totalAmountSold = parsePositiveBigInt(apiOrder.total_amount_sold, "total_amount_sold");
    const proceedsWithdrawn = parsePositiveBigInt(apiOrder.total_proceeds_withdrawn, "total_proceeds_withdrawn");
    ...
    if (status === "CLOSED") {
        order.closeDate = new Date(Math.min(parsedOrderId.orderKey.endTime, params.nowSeconds) * 1000);
    }
    return order;
}
