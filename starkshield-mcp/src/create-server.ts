...
export function createStarkshieldMcpServer(): McpServer {
  const server = new McpServer(
    { name: "starkshield", version: "1.0.0" },
    {
      instructions:
        "StarkShield executes Starknet operations via Starkzap inside this MCP process. Private keys never leave the server. Destructive tools return previews until confirm:true with previewId (unless STARKSHIELD_AUTO_CONFIRM=1). Use quantum_risk_scan for deterministic risk facts.",
    },
  );
