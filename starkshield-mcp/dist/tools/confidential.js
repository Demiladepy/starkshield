...
export function registerConfidentialTools(server) {
    server.registerTool("confidential_balance", {
    ...
    server.registerTool("confidential_fund", {
        description: "Shield public ERC20 into Tongo confidential balance. Destructive: preview + confirm.",
        inputSchema: {
            symbol: z.string(),
            amount: z.string(),
        ...
        if (!token)
            return jsonToolResult({ error: `Unknown token ${symbol}` });
        ...
        if (!autoConfirm()) {
            const stored = consumePreview(previewId ?? "", "confidential_fund");
    ...
    server.registerTool("confidential_send", {
        description: "Private confidential transfer to a recipient identity {x,y}. Destructive: preview + confirm.",
        inputSchema: {
        ...
        if (!autoConfirm()) {
            const stored = consumePreview(previewId ?? "", "confidential_send");
    ...
    server.registerTool("confidential_withdraw", {
        description: "Unshield confidential balance to a public Starknet address. Destructive: preview + confirm.",
        inputSchema: {
            symbol: z.string(),
            amount: z.string(),
        ...
        if (!autoConfirm()) {
            const stored = consumePreview(previewId ?? "", "confidential_withdraw");
            if (!stored || JSON.stringify(stored) !== JSON.stringify(payload)) {
