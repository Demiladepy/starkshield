...
export function registerSwapTools(server) {
    server.registerTool("swap_quote", {
    ...
    server.registerTool("swap_execute", {
        description: "Execute a swap via AVNU (default). Destructive: returns preview unless STARKSHIELD_AUTO_CONFIRM=1.",
        inputSchema: {
            tokenIn: z.string(),
            tokenOut: z.string(),
            amountIn: z.string(),
        ...
        if (!autoConfirm()) {
            const payload = consumePreview(previewId ?? "", "swap_execute");
            const same = payload &&
