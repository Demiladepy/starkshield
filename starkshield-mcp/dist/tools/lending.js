...
function tokens(net, collateral, debt) {
    const ct = resolvePresetToken(net, collateral);
    const dt = resolvePresetToken(net, debt);
    if (!ct || !dt)
        return { error: "Unknown collateral and/or debt token symbol." };
    return { ct, dt };
}
export function registerLendingTools(server) {
    server.registerTool("lending_markets", {
        ...
        return jsonToolResult(markets.map((m) => ({
            protocol: m.protocol,
    ...
    server.registerTool("lending_position", {
    ...
    server.registerTool("lending_health", {
    ...
    server.registerTool("lending_simulate", {
        description: "Simulate a lending action and return provider preflight + optional projected health (quoteHealth).",
        inputSchema: {
    ...
    const lendingMut = (name, kind, describe) => {
        server.registerTool(name, {
            ...
            if (!autoConfirm()) {
                const stored = consumePreview(previewId ?? "", kind);
            ...
            if (kind === "lending_deposit") {
                ...
                if (!sym)
                    return jsonToolResult({ error: "token (or collateralToken) is required for deposit." });
                const tok = resolvePresetToken(net, sym);
                if (!tok)
                    return jsonToolResult({ error: `Unknown token ${sym}` });
    ...
    lendingMut("lending_deposit", "lending_deposit", "Deposit into Vesu. Destructive: preview + confirm (or STARKSHIELD_AUTO_CONFIRM=1).");
    lendingMut("lending_borrow", "lending_borrow", "Borrow against collateral in Vesu. Destructive: preview + confirm.");
    lendingMut("lending_repay", "lending_repay", "Repay debt in Vesu. Destructive: preview + confirm.");
}
