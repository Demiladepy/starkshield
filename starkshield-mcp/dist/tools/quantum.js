import { z } from "zod";
import { assessQuantumRisk, } from "@starkshield/shared";
...
export function registerQuantumTools(server) {
    server.registerTool("quantum_risk_scan", {
        description: "Deterministic quantum-risk facts + optional Starknet RPC checks (no model inference in tool code).",
        inputSchema: {
        ...
        const onChain = chain === "starknet"
            ? await starknetOnChainChecks({ starknetTokenAddress, starknetAddress })
            : undefined;
        const result = assessQuantumRisk(asset, chain, walletType, onChain);
        return jsonToolResult(result);
    });
    server.registerTool("quantum_report", {
        ...
        const onChain = args.chain === "starknet"
            ? await starknetOnChainChecks({
                starknetTokenAddress: args.starknetTokenAddress,
                starknetAddress: args.starknetAddress,
            })
            : undefined;
        const result = assessQuantumRisk(args.asset, args.chain, args.walletType, onChain);
