...
export function registerTransferTools(server) {
    server.registerTool("transfer_send", {
        description: "Send a single ERC20 transfer. Destructive: returns a preview first unless STARKSHIELD_AUTO_CONFIRM=1.",
        inputSchema: {
            symbol: z.string(),
        ...
        if (!autoConfirm()) {
            const payload = consumePreview(previewId ?? "", "transfer_send");
    ...
    server.registerTool("transfer_batch", {
        description: "Send multiple ERC20 transfers in one transaction (same token). Destructive: preview + confirm.",
        inputSchema: {
            symbol: z.string(),
            transfers: z
                .array(z.object({
        ...
        if (!autoConfirm()) {
            const payload = consumePreview(previewId ?? "", "transfer_batch");
            if (!payload ||
        ...
        const tx = await w.transfer(token, normalized.map((x) => ({ to: x.to, amount: x.amount })));
        await tx.wait();
