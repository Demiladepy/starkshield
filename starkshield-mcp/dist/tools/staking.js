...
export function registerStakingTools(server) {
    server.registerTool("staking_pools", {
        ...
        return jsonToolResult(pools.map((p) => ({
            poolAddress: p.poolContract,
            token: p.token.symbol,
            amount: p.amount.toFormatted(),
        })));
    });
    server.registerTool("staking_enter", {
        description: "Enter or add to a staking pool with a preset token amount. Destructive: preview + confirm.",
        inputSchema: {
        ...
        if (!autoConfirm()) {
            const stored = consumePreview(previewId ?? "", "staking_enter");
    ...
    server.registerTool("staking_claim", {
        ...
        if (!autoConfirm()) {
            const stored = consumePreview(previewId ?? "", "staking_claim");
